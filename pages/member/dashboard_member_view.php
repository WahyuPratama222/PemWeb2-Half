﻿<div class="container-fluid py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="text-color-2 fw-bold mb-0">Dashboard Member</h4>
            <small class="text-muted-dark">Selamat datang, <?= escape(current_user()['name']) ?></small>
        </div>
            ...
            <div class="card bg-color-1 border border-secondary text-color-4 h-100">
                <div class="card-body d-flex align-items-center gap-3">
                    ...
                    <div>
                        ...
                        <div class="small text-muted-dark">Status Membership</div>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="card bg-color-1 border border-secondary text-color-4 h-100">
                <div class="card-body d-flex align-items-center gap-3">
                    <div class="p-3 rounded-3 bg-info bg-opacity-25">
                        <i class="bi bi-tags fs-4 text-color-3"></i>
                    </div>
                    <div>
                        <div class="fs-5 fw-bold"><?= $active_membership ? escape($active_membership['package_name']) : '-' ?></div>
                        <div class="small text-muted-dark">Paket Aktif</div>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="card-body">
                    ...
                    <?php if ($active_membership): ?>
                        <div class="p-3 rounded-3 border border-success border-opacity-25 bg-success bg-opacity-10">
                            <div class="fw-bold text-color-2 fs-6 mb-1">
                            ...
                            <div class="d-flex justify-content-between small">
                                <span class="text-muted-dark">Mulai</span>
                                <span class="text-color-4"><?= format_date($active_membership['start_date']) ?></span>
                            </div>
                            <div class="d-flex justify-content-between small">
                                <span class="text-muted-dark">Berakhir</span>
                                <span class="text-color-4"><?= format_date($active_membership['expiry_date']) ?></span>
                            </div>
                            <div class="d-flex justify-content-between small mt-1">
                                <span class="text-muted-dark">Sisa</span>
                                <span class="fw-bold <?= $days_remaining <= 7 ? 'text-color-3' : 'text-success' ?>">
                                    <?= $days_remaining ?> hari
                                </span>
                            </div>
                            <?php
                                ...
                                $percent    = $total_days > 0
                                    ...
                                    : 0;
                                $bar_color  = $percent > 50 ? 'bg-success' : ($percent > 20 ? 'bg-color-2' : 'bg-color-3');
                            ?>
                    ...
                    <?php else: ?>
                        <div class="text-center py-4">
                            ...
                            <p class="text-muted-dark small mb-3">Kamu belum memiliki membership aktif.</p>
                            <a href="<?= BASE_URL ?>controllers/member/packages_member.php"
                                class="btn btn-color-2 btn-sm fw-bold">
            ...
            <div class="card bg-color-1 border border-secondary text-color-4 h-100">
                <div class="card-body">
                    ...
                    <?php if (empty($recent_payments)): ?>
                        <div class="text-center py-4">
                            <i class="bi bi-receipt text-muted-dark fs-1 mb-2 d-block"></i>
                            <p class="text-muted-dark small mb-0">Belum ada riwayat pembayaran.</p>
                        </div>
                    <?php else: ?>
